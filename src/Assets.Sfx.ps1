...
function Initialize-Sounds {
    ...
    $defs = @{
        ...
        flame         = @(4, @(, @(0.20, 'n', 700, 1400, 0.55, 0.25)))
        lever         = @(6, @(@(0.04, 'n', 1500, 900, 0.7, 0.4), @(0.05, 'q', 1, 1, 0, 0), @(0.10, 'q', 220, 160, 0.6, 0.1)))
        spikes        = @(3, @(, @(0.12, 'q', 2800, 1400, 0.45, 0.0)))
        crusher       = @(5, @(@(0.04, 'n', 900, 500, 1.0, 0.9), @(0.22, 'n', 220, 60, 0.9, 0.0)))
        teleport      = @(7, @(@(0.18, 's', 300, 1800, 0.5, 0.6), @(0.20, 's', 1800, 500, 0.6, 0.0)))
        ...
        alarm         = @(9, @(@(0.30, 's', 520, 940, 0.5, 0.5), @(0.30, 's', 940, 520, 0.5, 0.5), @(0.30, 's', 520, 940, 0.5, 0.5), @(0.30, 's', 940, 520, 0.5, 0.5), @(0.30, 's', 520, 940, 0.5, 0.4), @(0.40, 's', 940, 520, 0.4, 0.0)))
        taser         = @(5, @(@(0.03, 'n', 5000, 7000, 0.45, 0.4), @(0.02, 'q', 1, 1, 0, 0), @(0.04, 'n', 6000, 4000, 0.45, 0.35), @(0.02, 'q', 1, 1, 0, 0), @(0.08, 'n', 7000, 3000, 0.4, 0.0)))
        bug_chirp     = @(4, @(@(0.04, 'q', 2400, 3100, 0.35, 0.35), @(0.03, 'q', 1, 1, 0, 0), @(0.04, 'q', 2600, 3300, 0.35, 0.3), @(0.03, 'q', 1, 1, 0, 0), @(0.05, 'q', 2400, 3000, 0.3, 0.0)))
        bug_squish    = @(5, @(@(0.05, 'n', 900, 300, 0.6, 0.5), @(0.12, 'n', 300, 90, 0.5, 0.0)))
