...
function Update-Abilities([double]$Tics, [hashtable]$In) {
    ...
    elseif ($In.Ability -eq 7) { Invoke-MineKey }
    elseif ($In.Ability) { Invoke-Ability ([int]$In.Ability) }
    Add-Privilege ($Tics / 45.0 * $(if ([int]$script:Stats.Policy -eq 0) { 1.3 } else { 1.0 }) * $(if (Test-Perk 'PSWindowsUpdate') { 1.5 } else { 1.0 }) * $(if ($script:TutorialMode) { 5.0 } else { 1.0 }))      # Restricted: nobody watches the logs
